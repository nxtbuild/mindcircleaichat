    ...
    >
      <CardHeader className="pb-2 relative">
        ...
        <CardTitle className="flex items-center gap-3">
          ...
          <span className="text-sm md:text-lg">{persona.name}</span>
        </CardTitle>
        <CardDescription className="!text-left !mt-4 ">
          <span className=" hidden md:block">{persona.title}</span>
        </CardDescription>
      </CardHeader>
      <CardContent className="pb-2 !text-left hidden md:block">
        <p className="text-sm text-muted-foreground">{persona.bio}</p>
      </CardContent>
      <CardFooter className="pt-2 flex gap-2 flex-wrap  hidden md:block">
        {persona.specialties.map((specialty, index) => (
          <Badge
            key={index}
            variant="outline"
            className="bg-dark-100 border-orange-500/20 text-xs mr-1"
          >
            {specialty}
          </Badge>
        ))}
